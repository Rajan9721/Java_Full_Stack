package classExercize.jdbc.ResultSet_Type;/*Seeing External Changes

Only a scroll-sensitive result set can see external changes to the underlying database, and it can only see the changes from external 
UPDATE operations. Changes from external DELETE or INSERT operations are never visible.
*/
...
class MyThread2 extends Thread{
	...
	public void run(){
		try{
			st.execute("UPDATE bank2 SET bal=bal+500");
			st.execute("COMMIT");
			st.close();
